...
namespace ZuiLib
{
	...
	ZVerticalLayout::ZVerticalLayout() : m_iSepHeight(0), m_uButtonState(0), m_bImmMode(false)
	{
		ptLastMouse.x = ptLastMouse.y = 0;
		memset(&m_rcNewPos,0,sizeof(m_rcNewPos));
	}

	ZVerticalLayout::~ZVerticalLayout(void)
	{
	}

	const char* ZVerticalLayout::GetClass() const
	{
		return "VerticalLayout";
	}

	void* ZVerticalLayout::GetInterface(const char* pstrName)
	{
		if( strcmp(pstrName, "VerticalLayout") == 0 ) 
			return static_cast<ZVerticalLayout*>(this);
		return ZContainer::GetInterface(pstrName);
	}

	uint32_t ZVerticalLayout::GetControlFlags() const
	{
	...
	void ZVerticalLayout::SetPos(RECT rc)
	{
		ZControl::SetPos(rc);
		rc = m_rcItem;
	...
	void ZVerticalLayout::DoPostPaint(ZRenerder* hDC, const RECT& rcPaint)
	{
	...
	void ZVerticalLayout::SetSepHeight(int iHeight)
	{
		m_iSepHeight = iHeight;
	}

	int ZVerticalLayout::GetSepHeight() const
	{
		return m_iSepHeight;
	}

	void ZVerticalLayout::SetSepImmMode(bool bImmediately)
	{
	...
	bool ZVerticalLayout::IsSepImmMode() const
	{
		return m_bImmMode;
	}

	void ZVerticalLayout::SetAttribute(const char* pstrName, const char* pstrValue)
	{
		if( strcmp(pstrName, "sepheight") == 0 ) 
			SetSepHeight(atoi(pstrValue));
	...
	void ZVerticalLayout::DoEvent(TEventUI& event)
	{
		if( m_iSepHeight != 0 ) {
	...
	RECT ZVerticalLayout::GetThumbRect(bool bUseNew) const
	{
		if( (m_uButtonState & UISTATE_CAPTURED) != 0 && bUseNew) {
			if( m_iSepHeight >= 0 ) 
			{
				RECT rc={m_rcNewPos.left, ZMAX(m_rcNewPos.bottom - m_iSepHeight, m_rcNewPos.top), 
				m_rcNewPos.right, m_rcNewPos.bottom};
				return rc;
			}
			else 
			{
				RECT rc={m_rcNewPos.left, m_rcNewPos.top, m_rcNewPos.right, 
					ZMIN(m_rcNewPos.top - m_iSepHeight, m_rcNewPos.bottom)};
				return rc;
			}
		}
		else {
			if( m_iSepHeight >= 0 )
			{
				RECT rc={m_rcItem.left, ZMAX(m_rcItem.bottom - m_iSepHeight, m_rcItem.top), m_rcItem.right, 
					m_rcItem.bottom};
				return rc;
			}
			else
			{
				RECT rc={m_rcItem.left, m_rcItem.top, m_rcItem.right, 
					ZMIN(m_rcItem.top - m_iSepHeight, m_rcItem.bottom)};
				return rc;
			}
		}
	}
