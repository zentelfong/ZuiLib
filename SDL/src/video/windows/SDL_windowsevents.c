...
WIN_WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	if(msg==WM_CREATE)
	{
		SetTimer(hwnd,RENDER_TIMER_ID,1000/60,NULL);
	}
	...
	case WM_TIMER:
		...
		{
			typedef HRESULT (*WindowRenderHandle)(SDL_Window* wnd,const RECT* rc);
			WindowRenderHandle renderHandle=(WindowRenderHandle)SDL_GetWindowData(data->window,"EV_Render");
			RECT rc={0};
			GetClientRect(hwnd,&rc);
			if (renderHandle)
				return renderHandle(data->window,&rc);
		}
		break;

	case WM_NCHITTEST:
		{
			typedef HRESULT (*WindowHitTestHandle)(SDL_Window* wnd,HWND hWnd,WPARAM wparam,LPARAM lparam);
			WindowHitTestHandle hitTestHandle=(WindowHitTestHandle)SDL_GetWindowData(data->window,"EV_HitTest");

			if (hitTestHandle)
				return hitTestHandle(data->window,hwnd,wParam,lParam);
			else
				return HTCLIENT;
		}
		break;
