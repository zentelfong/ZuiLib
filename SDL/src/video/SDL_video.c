...
void
SDL_SetWindowTransparent(SDL_Window * window, SDL_bool transparent)
{
    CHECK_WINDOW_MAGIC(window, );
    if (!(window->flags & SDL_WINDOW_FULLSCREEN)) {
		const int want = (transparent != SDL_FALSE);  /* normalize the flag. */
        const int have = ((window->flags & SDL_WINDOW_TRANSPARENT) != 0);
        if ((want != have) && (_this->SetWindowTransparent)) {
            if (want) {
                window->flags &= ~SDL_WINDOW_TRANSPARENT;
            } else {
                window->flags |= SDL_WINDOW_TRANSPARENT;
            }
            _this->SetWindowTransparent(_this, window, (SDL_bool) want);
        }
    }
}
